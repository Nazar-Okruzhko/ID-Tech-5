...
def find_buffer_marker(data, start_offset=0):
    """Find marker but exclude 00 00 and 00 01 immediately after"""

    pos = start_offset

    while True:

        pos = data.find(BUFFER_MARKER, pos)

        if pos == -1:
            return -1

        # Read next two bytes after marker
        next1 = data[pos+3]
        next2 = data[pos+4]
        ...
        # 01 00 00 00 00
        # 01 00 00 00 01
        if next1 == 0x00 and (next2 == 0x00 or next2 == 0x01):
            pos += 1
            continue
